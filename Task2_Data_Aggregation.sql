...
SELECT
    ...
    CONCAT(
        ROUND(
            100 * (COUNT(*) - LAG(COUNT(*)) OVER (ORDER BY YEAR(hire_date)))
            / LAG(COUNT(*)) OVER (ORDER BY YEAR(hire_date)),
        1),
        '%'
    ) AS Growth_Rate
FROM employees
...
-- Exercise 4: Salary Distribution Report (Sales)
...
SELECT
    ...
    LPAD(
        CONCAT(
            ROUND(
                100 * COUNT(*) / SUM(COUNT(*)) OVER (PARTITION BY d.dept_name),
            1),
            '%'
        ),
    6) AS Distribution
...
-- Check null department assignments
SELECT COUNT(*)
FROM dept_emp
